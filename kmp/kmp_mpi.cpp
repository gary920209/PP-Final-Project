// kmp_mpi.cpp
// mpicxx -O3 -std=c++17 kmp_mpi.cpp -o kmp_mpi
#include <mpi.h>
#include <bits/stdc++.h>
using namespace std;
...
static long long kmp_search_count_owned(
    ...
    for (int i = 0; i < n; i++) {
        while (j > 0 && local_text[i] != pat[j]) {
            j = failure[j - 1];
        }
        if (local_text[i] == pat[j]) {
            j++;
        }
        if (j == m) {
            long long match_pos_global = global_local_start + (i - m + 1);
...
int main(int argc, char** argv) {
    ...
    // Build failure function (same on all ranks)
    ...
    double t0 = MPI_Wtime();
    long long local_count = kmp_search_count_owned(local, read_start, pat, failure, my_start, my_end);
    double t1 = MPI_Wtime();
